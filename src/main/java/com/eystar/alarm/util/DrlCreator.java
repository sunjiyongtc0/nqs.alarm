        ...
        for(int i=0;i<3;i++){
            if(i==0) {
                pdb.newRule().name(record.getStr("id") + "_" + i)
                        ...
                        .lhs()
                        .and()
                        .pattern("JSONObject").id("$j", false).constraint("task_type_name!=\"" + record.getStr("task_type_name") + "\"").end()
//                        .and().pattern("RuleTreeOrbit").constraint("ruleOrbit contains \"rule_1\"").end().end()
                        .end()
                        .end()
...
//                        .lhs()
//                        .and()
//                        .pattern("JSONObject").id("$j", false).constraint("alarm_info==\"" + record.getStr("id") + "\" ||alarm_info==\"\"").end()
//                        .end()
//                        .end()
//                        .rhs("").end();
            }else if(i==2){
                pdb.newRule().name(record.getStr("id") + "_" + i)
                        ...
                        .lhs()
                        .and()
                        .pattern("JSONObject").id("$j", false).constraint("$j.containsKey(\""+index+"\") &&"+index+">0").end()
                        .end()
                        .end()
...
//        pdb.newFunction().returnType("boolean").name("getB").body("")
...
//            String path=PathKit.getRootClassPath()+ File.separator+"rules"+ File.separator+fileName+".drl";
